create or replace function public.prepare_manual_communication(p_approval_id uuid, p_channel text, p_template_key text)
...
declare
  ...
  v_stage smallint;
  v_channel_has_stage boolean := false;
  v_template_key text;
...
begin
  ...
  if p_template_key not in ('approval_initial', 'reminder_1', 'reminder_2', 'final_notice') then
    raise exception 'Modelo de mensagem inválido.';
  end if;
  ...
  if v_approval.status <> 'awaiting_approval' then
  ...
  v_stage := coalesce(v_approval.followup_stage, 0);
  v_start_at := coalesce(v_approval.sent_at, v_approval.created_at);

  if v_approval.due_at is not null and now() >= v_approval.due_at then
    if not v_settings.final_notice_enabled then
      raise exception 'O prazo da aprovação terminou e o aviso final está desativado nas configurações.';
    end if;
    v_template_key := 'final_notice';
  elsif v_stage <= 0 then
    select exists (
      select 1
      from public.message_queue mq
      join public.message_templates mt on mt.id = mq.template_id
      where mq.approval_id = v_approval.id
        and mq.channel = p_channel
        and mq.status in ('sent', 'delivered', 'read')
        and mt.template_key = 'approval_initial'
    ) into v_channel_has_stage;
    v_template_key := case when v_channel_has_stage then 'reminder_1' else 'approval_initial' end;
    ...
    select exists (
      ...
      where mq.approval_id = v_approval.id
        and mq.channel = p_channel
        and mq.status in ('sent', 'delivered', 'read')
        and mt.template_key = 'reminder_1'
    ) into v_channel_has_stage;
    v_template_key := case when v_channel_has_stage then 'reminder_2' else 'reminder_1' end;
    ...
    select exists (
      ...
      where mq.approval_id = v_approval.id
        and mq.channel = p_channel
        and mq.status in ('sent', 'delivered', 'read')
        and mt.template_key = 'reminder_2'
    ) into v_channel_has_stage;
    v_template_key := case when v_channel_has_stage then 'final_notice' else 'reminder_2' end;
  else
    v_template_key := 'final_notice';
  end if;
  ...
  select exists (
    ...
    where mq.approval_id = v_approval.id
      and mq.channel = p_channel
      and mq.status in ('sent', 'delivered', 'read')
      and mt.template_key = v_template_key
  ) into v_channel_has_stage;

  if v_channel_has_stage then
    if v_template_key = 'final_notice' then
      raise exception 'O aviso final deste canal já foi registrado como enviado.';
    else
      raise exception 'Esta etapa deste canal já foi registrada como enviada.';
    end if;
  end if;

  if v_template_key = 'reminder_1' and v_stage < 1 then
    v_available_at := v_start_at + make_interval(hours => v_settings.reminder_1_hours);
    if now() < v_available_at then
      raise exception 'O primeiro lembrete estará disponível em %.', to_char(v_available_at at time zone 'America/Cuiaba', 'DD/MM/YYYY HH24:MI');
    end if;
  elsif v_template_key = 'reminder_2' and v_stage < 2 then
    v_available_at := v_start_at + make_interval(hours => v_settings.reminder_2_hours);
    if now() < v_available_at then
      raise exception 'O segundo lembrete estará disponível em %.', to_char(v_available_at at time zone 'America/Cuiaba', 'DD/MM/YYYY HH24:MI');
    end if;
  elsif v_template_key = 'final_notice' then
    if not v_settings.final_notice_enabled then
      raise exception 'O aviso final está desativado nas configurações.';
    end if;
    if v_approval.due_at is null then
      raise exception 'A aprovação não possui prazo definido.';
    end if;
    if now() < v_approval.due_at then
      raise exception 'O aviso final estará disponível após o prazo, em %.', to_char(v_approval.due_at at time zone 'America/Cuiaba', 'DD/MM/YYYY HH24:MI');
    end if;
  end if;

  select * into v_template
  from public.message_templates
  where template_key = v_template_key and channel = p_channel and is_active = true;
...
begin
  ...
  if v_template_key = 'approval_initial' then
    if v_stage > 0 then
      raise exception 'A aprovação já avançou para a etapa de lembretes. Prepare uma nova comunicação.';
    end if;
  elsif v_template_key = 'reminder_1' then
    if v_approval.due_at is not null and now() >= v_approval.due_at then
      raise exception 'O prazo da aprovação já terminou. Prepare o aviso final.';
    end if;
    if v_stage > 1 then
      raise exception 'A aprovação já avançou além do primeiro lembrete. Prepare uma nova comunicação.';
    end if;
    if v_stage < 1 then
      v_available_at := v_start_at + make_interval(hours => v_settings.reminder_1_hours);
      if now() < v_available_at then
    ...
    if v_stage < 1 then
      raise exception 'O primeiro lembrete ainda não foi concluído.';
    end if;
    if v_stage > 2 then
      raise exception 'A aprovação já avançou além do segundo lembrete. Prepare uma nova comunicação.';
    end if;
    if v_stage < 2 then
      v_available_at := v_start_at + make_interval(hours => v_settings.reminder_2_hours);
      if now() < v_available_at then
  ...
  if v_target_stage > 0 then
    ...
    set followup_stage = greatest(coalesce(followup_stage, 0), v_target_stage)
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.agency_settings'::regclass
      and conname = 'agency_settings_reminder_order_check'
  ) then
    alter table public.agency_settings
      add constraint agency_settings_reminder_order_check
      check (reminder_2_hours > reminder_1_hours);
  end if;
end $$;
