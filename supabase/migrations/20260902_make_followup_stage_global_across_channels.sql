  ...
  if v_stage <= 0 then
    ...
    select exists (
      ...
      where mq.approval_id = v_approval.id
        and mq.channel = p_channel
        and mq.status in ('sent', 'delivered', 'read')
        and mt.template_key = 'final_notice'
    ) into v_channel_has_stage;
...
begin
  ...
  if not found then raise exception 'Aprovação não encontrada.'; end if;
  if v_approval.status <> 'awaiting_approval' then raise exception 'Esta aprovação não está mais aguardando retorno.'; end if;
  ...
  if v_template_key = 'approval_initial' and v_stage <> 0 then
    raise exception 'A aprovação já avançou para a etapa de lembretes.';
  elsif v_template_key = 'reminder_1' and v_stage not in (0, 1) then
    raise exception 'A aprovação já avançou além do primeiro lembrete.';
  elsif v_template_key = 'reminder_2' and v_stage not in (1, 2) then
    raise exception 'O segundo lembrete não corresponde à etapa atual da aprovação.';
  elsif v_template_key = 'final_notice' and v_stage not in (2, 3) then
    raise exception 'O aviso final não corresponde à etapa atual da aprovação.';
  end if;
