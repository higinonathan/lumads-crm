...
(() => {
  ...
  const clientMark = (client, cls = '') => `<div class="company-dot${client && client.logo ? ' has-logo' : ''}${cls ? ' ' + cls : ''}">${client && client.logo ? `<img src="${client.logo}" alt="Logo de ${text(client.name)}">` : text(initials(client.name))}</div>`;
  const clientCell = client => `<div class="client-cell">${clientMark(client)}<div><div class="client-name">${text(client.name)}</div><div class="client-company">${text(client.company)}</div></div></div>`;
  ...
  function renderClients() {
    $('#dynamicContent').innerHTML = `<section class="client-grid">${state.clients.map(client => { const sum = clientSummary(client); return `<button class="client-card" data-action="client-detail" data-client="${client.id}"><div class="client-card-top">${clientMark(client, 'client-card-mark')}<div><h3>${text(client.name)}</h3><div class="company">${text(client.company)}</div></div></div><div class="client-meta"><span>${text(client.whatsapp)}</span><span>${text(client.email)}</span></div><div class="client-card-foot"><span><b class="pending-count">${sum.pending}</b> pendente${sum.pending === 1 ? '' : 's'}</span>${badge(sum.status)}</div></button>`; }).join('')}</section>`;
  }
  ...
  function bindClientLogoField() {
    ...
    input.addEventListener('change', () => {
      ...
      if (file.size > 1.5 * 1024 * 1024) { modalError('Selecione uma imagem de até 1,5 MB.'); input.value = ''; return; }
      const reader = new FileReader();
      reader.onload = () => { const data = String(reader.result); $('#clientLogo').value = data; $('#clientLogoPreview').classList.add('has-logo'); $('#clientLogoPreview').innerHTML = `<img src="${data}" alt="Pré-visualização do logotipo">`; $('#clientLogoRemove').classList.remove('is-hidden'); const err = $('#modalError'); if (err) err.hidden = true; };
      reader.readAsDataURL(file);
    });
    $('#clientLogoRemove').addEventListener('click', () => { $('#clientLogo').value = ''; input.value = ''; const name = $('#clientName').value.trim(); $('#clientLogoPreview').classList.remove('has-logo'); $('#clientLogoPreview').innerHTML = `<span>${text(name ? initials(name) : 'Logo')}</span>`; $('#clientLogoRemove').classList.add('is-hidden'); });
  }
  function saveClientForm() { const id = $('#editClientId').value; const values = { name: $('#clientName').value.trim(), company: $('#clientCompany').value.trim(), contact: $('#clientContact').value.trim(), whatsapp: $('#clientPhone').value.trim(), email: $('#clientEmail').value.trim(), notes: $('#clientNotes').value.trim(), logo: $('#clientLogo').value || '' }; if (!values.name || !values.company || !values.contact || !values.whatsapp || !values.email) return modalError('Preencha todos os campos obrigatórios.'); if (id) { Object.assign(clientById(id), values); showToast('Cliente atualizado com sucesso.'); } else { state.clients.push({ id: newId('client'), ...values, createdAt: new Date().toISOString() }); showToast('Cliente criado com sucesso.'); } save(); closeModal(); if (id) renderClientDetail(id); else navigate('Clientes'); }
