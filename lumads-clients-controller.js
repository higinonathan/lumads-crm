...
(() => {
  ...
  const legacyRoot = document.querySelector('#dynamicContent');
  if (!crm || !legacyRoot) return;

  const approvedRoot = document.createElement('div');
  approvedRoot.id = 'clientsApprovedContent';
  approvedRoot.hidden = true;
  legacyRoot.insertAdjacentElement('afterend', approvedRoot);
  ...
  const summaryFor = clientId => {
    ...
    const latest = [...records].sort((a, b) => new Date(b.statusChangedAt || b.updatedAt || b.createdAt) - new Date(a.statusChangedAt || a.updatedAt || a.createdAt))[0];
    return { records, pending, completed, latestAt: latest ? (latest.statusChangedAt || latest.updatedAt || latest.createdAt) : null };
  };
  ...
  function writeClientUrl(clientId, { replace = false } = {}) {
    const url = new URL(window.location.href);
    if (clientId) url.searchParams.set('client', clientId);
    else url.searchParams.delete('client');
    url.hash = 'clients';
    history[replace ? 'replaceState' : 'pushState']({ clientId: clientId || null }, '', url);
  }

  function clearClientQueryPreservingLocation() {
    const url = new URL(window.location.href);
  ...
  function setListHeader() {
    ...
    const primary = document.querySelector('#headerActions .primary');
    if (primary) {
      primary.dataset.action = 'new-client';
      delete primary.dataset.client;
      primary.textContent = '+ Novo cliente';
    }
  }

  function setDetailHeader(clientId) {
    document.querySelector('#clientDetailHeaderBack')?.remove();
    const eyebrow = document.querySelector('#pageEyebrow');
    const title = document.querySelector('#pageTitle');
    if (eyebrow) {
      eyebrow.insertAdjacentHTML('beforebegin', '<button class="back-link client-detail-back-approved" id="clientDetailHeaderBack" data-clients-approved-back>← Voltar para clientes</button>');
      eyebrow.textContent = 'Base de clientes';
    }
    if (title) title.textContent = 'Detalhe do cliente';
    const primary = document.querySelector('#headerActions .primary');
    if (primary) {
      primary.dataset.action = 'new-approval';
      primary.dataset.client = clientId;
      primary.textContent = '+ Nova aprovação';
    }
  }
  ...
  function clientCard(client) {
    ...
    return `<article class="clients-ag-card" data-client-card data-client="${escapeHtml(client.id)}" data-active="${active}" data-search="${escapeHtml(search)}" data-name="${escapeHtml((client.name || '').toLowerCase())}" tabindex="0" role="button" aria-label="Abrir cliente ${escapeHtml(client.name)}">
      ...
      <div class="clients-ag-card-foot"><div class="clients-ag-foot-state"><strong>${summary.pending} pendente${summary.pending === 1 ? '' : 's'}</strong><span class="clients-ag-active-pill ${active ? '' : 'archived'}">• ${active ? 'Ativo' : 'Arquivado'}</span></div><div class="clients-ag-actions">${active && client.whatsapp ? `<button class="clients-ag-round whatsapp" type="button" data-action="whatsapp-client" data-client="${escapeHtml(client.id)}" aria-label="Abrir WhatsApp">${whatsappIcon}</button>` : ''}</div></div>
    </article>`;
  }

  function applyListState() {
    const root = approvedRoot.querySelector('.clients-ag-page');
    if (!root) return;
  ...
  async function renderDetail(clientId) {
    ...
    if (!client) {
      if (view.loading) {
        approvedRoot.innerHTML = '<div class="clients-ag-detail-loading" aria-live="polite">Carregando cliente…</div>';
        return;
      }
      writeClientUrl('', { replace: true });
      renderList();
      return;
    }

    setDetailHeader(clientId);
    const records = view.approvals.filter(item => item.clientId === clientId);
  ...
  function renderRoute() {
    ...
    if (clientId) void renderDetail(clientId);
    else renderList();
  }
  ...
  async function refreshData() {
    ...
    view.loading = true;
    try {
      const [clients, approvals] = await Promise.all([loadClients(), loadApprovals()]);
      view.clients = clients || [];
      view.approvals = approvals || [];
      renderRoute();
    } catch (error) {
      console.error('Não foi possível carregar a página Clientes.', error);
      approvedRoot.innerHTML = '<div class="clients-ag-no-results"><strong>Não foi possível carregar os clientes</strong><span>Tente novamente em alguns instantes.</span></div>';
    } finally {
      view.loading = false;
  ...
  function activateClients({ forceRefresh = false } = {}) {
    const authenticatedVisible = !crm.hidden && crm.style.display !== 'none';
    const active = crm.dataset.page === 'Clientes' && authenticatedVisible;
    view.active = active;
    approvedRoot.hidden = !active;
    if (active) {
      legacyRoot.style.display = 'none';
      if (forceRefresh || !view.clients.length) void refreshData();
      else renderRoute();
    } else {
  ...
  const legacyObserver = new MutationObserver(() => scheduleRefresh());
  legacyObserver.observe(legacyRoot, { childList: true, subtree: false });

  const crmObserver = new MutationObserver(() => activateClients());
  crmObserver.observe(crm, { attributes: true, attributeFilter: ['data-page', 'hidden', 'style'] });

  document.addEventListener('input', event => {
    const input = event.target.closest?.('[data-clients-ag-search]');
    if (!input || !approvedRoot.contains(input)) return;
  ...
  document.addEventListener('click', event => {
    ...
    if (nav) {
      clearClientQueryPreservingLocation();
      if (nav.dataset.page === 'Clientes') queueMicrotask(() => activateClients({ forceRefresh: true }));
      return;
    }
    ...
    const back = event.target.closest?.('[data-clients-approved-back]');
    if (back) {
    ...
    if (filter && approvedRoot.contains(filter)) {
      event.preventDefault();
      event.stopImmediatePropagation();
      view.filter = filter.dataset.clientsAgFilter;
      applyListState();
      return;
    }

    const sort = event.target.closest?.('[data-clients-ag-sort]');
    if (sort && approvedRoot.contains(sort)) {
      event.preventDefault();
      event.stopImmediatePropagation();
    ...
    if (more && approvedRoot.contains(more)) {
      event.preventDefault();
      event.stopImmediatePropagation();
      const menu = more.nextElementSibling;
      approvedRoot.querySelectorAll('.clients-ag-menu').forEach(item => { if (item !== menu) item.hidden = true; });
      if (menu) menu.hidden = !menu.hidden;
      return;
    }

    if (!event.target.closest?.('.clients-ag-menu')) approvedRoot.querySelectorAll('.clients-ag-menu').forEach(item => { item.hidden = true; });

    const card = event.target.closest?.('[data-client-card]');
    if (!card || !approvedRoot.contains(card)) return;
    if (event.target.closest('button, a, input, select, textarea, label')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const clientId = card.dataset.client;
    if (!clientId) return;
    writeClientUrl(clientId);
    void renderDetail(clientId);
  }, true);

  document.addEventListener('keydown', event => {
    const card = event.target.closest?.('[data-client-card]');
    if (!card || !approvedRoot.contains(card) || (event.key !== 'Enter' && event.key !== ' ')) return;
    if (event.target.closest('button, a, input, select, textarea, label')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const clientId = card.dataset.client;
  ...
  window.addEventListener('hashchange', () => activateClients({ forceRefresh: window.location.hash === '#clients' }));
  activateClients();
})();
